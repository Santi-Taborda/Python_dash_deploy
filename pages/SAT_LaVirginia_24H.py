...
def obtener_datos():
    ...
    # Consultas SQL
    query1 = "SELECT idEstacion, IdTiempoRegistro, Valor FROM factmonitoreo WHERE idEstacion IN (114,113,17,20,21) AND IdVariable in (2) AND IdTiempoRegistro BETWEEN %s AND %s AND Valor IS NOT NULL"
    query2 = "SELECT IdEstacion, Estacion, Latitud, Longitud FROM dimestacion WHERE idEstacion IN (114,113,17,20,21)"
...
register_page(__name__, name="SAT_LA_VIRGINIA_24H", path='/SAT/LA_VIRGINIA/24H')

layout= dbc.Container(children=[
    html.Div(
        children=[
    html.H1("Monitor SAT LA VIRGINIA 24 HORAS", style={'textAlign': 'left', 'color': '#0d6efd', 'margin-left':'20px', 'padding':'10px'}),
    html.Hr()],
...
@callback(
    Output("layer_SAT_LaVirginia_24H", "children"),
    [Input("layer_SAT_LaVirginia_24H", "id"),
