...
def obtener_datos():
    ...
    # Consultas SQL
    query1 = "SELECT idEstacion, date(IdTiempoRegistro) as IdTiempoRegistro, round(sum(Valor),2) as Valor FROM factmonitoreo WHERE idEstacion IN (1,4,6,10,11,12,13,14,15,24,25,28,29,30,78,78,79,82,91,95,96,97,98,114,51,122) AND IdVariable in (2) AND IdTiempoRegistro BETWEEN %s AND %s AND Valor IS NOT NULL GROUP BY idEstacion, date(IdTiempoRegistro)"
...
register_page(__name__, name="SAT_PEREIRA_40D", path='/SAT/PEREIRA/40D')
    ...
    dbc.Row(children=[
        dbc.Col([
            dl.Map(center=[4.8074, -75.7345], zoom=12, children=[
                dl.TileLayer(),
                dl.LayerGroup(id="layer_SAT_Pereira_40D")
...
@callback(
    ...
    [Input("layer_SAT_Pereira_40D", "id"),
