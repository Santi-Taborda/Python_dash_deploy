...
def obtener_estaciones():
    ...
    query3= "SELECT Estacion, Estado FROM estaciones where IdTipoEstacion IN(1,2,3,7,9,10,11,12,14) and Estado IN(1)"
    ...
    estaciones = pd.merge(dimestacion, dimestado, on="Estacion", how="inner")
    return estaciones

def obtener_dato_estacion(estacion):
    ...
    engine = create_engine(env.get('DB_URL'), echo=True)
    # Consultas SQL
    query1 = "SELECT idEstacion, IdTiempoRegistro, IdVariable, Valor FROM factmonitoreo WHERE IdTiempoRegistro BETWEEN %s AND %s AND idEstacion in(%s) AND Valor IS NOT NULL"
    datos_tabla = pd.read_sql(query1, engine,  params=(hace_2_dias_str, hora_actual_str, estacion))
...
def update_markers(_):
    ...
    for _, row in estaciones.iterrows():
        ...
        marker= dl.Marker(
            ...
            icon=icon, 
            id=row['Estacion'],
            children=[dl.Tooltip(name),dl.Popup(id=f"popup-{row['Estacion']}", children="", maxWidth=700, minWidth=200)])
...
def display_popup(_, *args):
    ...
    datos=obtener_dato_estacion(estacion["idEstacion"].iloc[0])
