...
def obtener_datos():
    ...
    # Consultas SQL
    query1 = "SELECT idEstacion, IdTiempoRegistro, Valor FROM factmonitoreo WHERE idEstacion IN (83,106,107) AND IdVariable in (2) AND IdTiempoRegistro BETWEEN %s AND %s AND Valor IS NOT NULL"
    query2 = "SELECT IdEstacion, Estacion, Latitud, Longitud FROM dimestacion WHERE idEstacion IN (83,106,107)"
...
register_page(__name__, name="SAT_LA_CELIA_24H", path='/SAT/LA_CELIA/24H')

layout= dbc.Container(children=[
    html.Div(
        children=[
    html.H1("Monitor SAT LA CELIA 24 HORAS", style={'textAlign': 'left', 'color': '#0d6efd', 'margin-left':'20px', 'padding':'10px'}),
    html.Hr()],
    style={'background-color':'AliceBlue'},
    ),
    dbc.Row(children=[
        dbc.Col([
            dl.Map(center=[5.0035, -76.0032], zoom=12, children=[
                dl.TileLayer(),
                dl.LayerGroup(id="layer_SAT_LaCelia_24H")
...
@callback(
    Output("layer_SAT_LaCelia_24H", "children"),
    [Input("layer_SAT_LaCelia_24H", "id"),
