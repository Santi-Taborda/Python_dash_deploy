...
def obtener_datos():
    ...
    # Consultas SQL
    query1 = "SELECT idEstacion, ï»¿IdTiempoRegistro as IdTiempoRegistro, Valor FROM Bomberos_prueba WHERE idEstacion IN (1,26,98, 30) AND IdVariable in (2) AND Valor IS NOT NULL"
    query2 = "SELECT IdEstacion, Estacion, Latitud, Longitud FROM dimestacion WHERE idEstacion IN (1,26,98, 30)"
...
register_page(__name__, name="SAT_DIDACTICO_BOMBEROS", path='/SAT/BOMBEROS_CAPACITACION/24H')

layout= dbc.Container(children=[
    html.Div(
        children=[
    html.H1("Monitor SAT BOMBEROS CAPACITACIÓN 24 HORAS", style={'textAlign': 'left', 'color': '#0d6efd', 'margin-left':'20px', 'padding':'10px'}),
    html.Hr()],
    style={'background-color':'AliceBlue'},
    ),
    dbc.Row(children=[
        dbc.Col([
            dl.Map(center=[4.8074, -75.7345], zoom=12, children=[
                dl.TileLayer(),
                dl.LayerGroup(id="layer_BOMBEROS_CAPACITACION_24H")
        ...
        dbc.CardBody([
            ...
            dbc.Row(children=[
                ...
                dbc.Col(html.P("<25 mm", className="mb-0"), width="auto")
            ], align="left"),
...
@callback(
    Output("layer_BOMBEROS_CAPACITACION_24H", "children"),
    [Input("layer_BOMBEROS_CAPACITACION_24H", "id"),
