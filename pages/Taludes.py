...
#CAMBIOS NECESARIOS PARA HACER ADJUNTAMIENTO POR NODO

"""             if state==3:
                icon_momo=icon_red
                nodo_state=3
            if state==2 and state>= nodo_state:
                icon_momo=icon_orange
                nodo_state=2
            if state==1 and nodo_state ==1:
                icon_momo=icon_green
            if state==0 and nodo_state== 1:
                icon_momo=icon_black
                nodo_state=0 """
                                ...
                                children=[
                                        ...
                                        dbc.CardBody([
                                            ...
                                            dbc.Row([
                                                dbc.Col(
                                                    html.Div(
                                                        style={"height": "20px", "width": "20px", "text-align":"center", "margin":"3px", "backgroundColor": row['color_momo']},
                                                        title=name  # Esto es para el tooltip
                                                    ),
                                                    width=1, 
                                                )
...
""" def obtener_datos(datos_tabla):
    ...
    for i, momo in enumerate(momos):
        ...
        datos_tabla.at[i, 'state'] = state
        if state==0:
            datos_tabla.at[i, 'color_momo'] = '#000000'
        elif state==1:
            datos_tabla.at[i, 'color_momo'] = '#1FFF00'
        elif state==2:
            datos_tabla.at[i, 'color_momo'] = '#FF9700'
        elif state==3:
            datos_tabla.at[i, 'color_momo'] = '#FF0000'
